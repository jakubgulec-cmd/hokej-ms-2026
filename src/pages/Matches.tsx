...
function getPointsLabel(pred: Prediction, match: Match) {
  ...
  if (isExact) return { text: 'Přesný tip', sub: `+${bodyStr(9 * mult)}`, color: 'text-yellow-400', bg: 'bg-yellow-500/10 border-yellow-700/50' };
  ...
  if (correctDiff) return { text: 'Výsledek + rozdíl', sub: `+${bodyStr(4 * mult)}`, color: 'text-blue-400', bg: 'bg-blue-500/10 border-blue-700/50' };

  return { text: 'Správný výsledek', sub: `+${bodyStr(1 * mult)}`, color: 'text-green-400', bg: 'bg-green-500/10 border-green-700/50' };
}
